...
function parseFile(ab, name) {
  ...
  if (name.toLowerCase().endsWith(".epub")) {
    ...
    text = result.text;
    // GBK = 簡體 → 自動轉繁體
    if (result.enc === "gbk") text = s2t(text);
  }
  return text;
}
...
export default function App() {
  ...
  if (view === "library") return (
    <div style={{ minHeight:"100vh", background:lbg, color:ltc, fontFamily:"Georgia,'Noto Serif TC',serif", paddingBottom:34 }}>
      ...
      <div style={{ padding:"10px 16px", textAlign:"center", fontSize:12, color:lmu }}>長按書籍可重新命名或刪除</div>
    </div>
  );
  ...
  return (
    <div style={{ background:rbg, color:rtc, fontFamily:"Georgia,'Noto Serif TC',serif", display:"flex", flexDirection:"column", height:"100vh", width:"100vw", overflow:"hidden", filter:`brightness(${bright})` }}
      ...
      {gestureHint && <div style={{ position:"fixed", top:"42%", left:"50%", transform:"translate(-50%,-50%)", background:"rgba(0,0,0,0.65)", color:"#fff", borderRadius:12, padding:"10px 20px", fontSize:16, zIndex:300, pointerEvents:"none" }}>{gestureHint}</div>}
      ...
      {/* 內文 — 禁止滾動，固定顯示 */}
      <div ref={rdr} style={{ flex:1, overflow:"hidden", padding:"24px 24px", lineHeight:1.95, fontSize:fs, color:rtc, whiteSpace:"pre-wrap", wordBreak:"break-word" }}>
        {pageText}
      </div>

      {/* 底部進度 — 固定高度避免被遮 */}
      <div style={{ padding:"12px 20px", paddingBottom:"max(24px, env(safe-area-inset-bottom))", borderTop:`1px solid ${rbd}`, background:rhd, display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0 }}>
