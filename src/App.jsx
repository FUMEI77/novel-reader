...
const CHANGELOG = [
  { version: "v1.3.0", date: "2026-05", notes: ["自動偵測簡體中文並轉換為繁體", "拿掉手動簡繁切換鍵", "底部改為閱讀進度顯示", "修正左右留白不對稱"] },
...
function isSimplifiedChinese(text) {
  ...
  let simplified = 0, total = 0;
  for (const c of sample) {
    if (S2T_MAP[c] !== undefined) { simplified++; total++; }
    else if (S2T_MAP[c] === undefined && c.charCodeAt(0) > 0x4E00 && c.charCodeAt(0) < 0x9FFF) total++;
  }
  return total > 50 && (simplified / total) > 0.15;
}
...
function decodeBuffer(ab) {
  ...
  try {
    ...
    if (t.includes("â") || t.includes("ã") || t.includes("�")) throw new Error();
    return t;
  } catch {
...
function parseFile(ab, name) {
  ...
  // 自動偵測簡體並轉繁體
  if (isSimplifiedChinese(text)) text = s2t(text);
  return text;
}
...
export default function App() {
  ...
  function onTouchEnd(e) {
    ...
    if (fingers === 2 && absDx < 20 && absDy < 20) {
      if (gestures.prevPage === "two_tap") { goPage(page - 1); showHint("← 上一頁"); } return;
    }
    if (fingers !== 1) return;
    ...
    if (absDx < 15 && absDy < 15 && gestures.nextPage === "tap_right" && endX > W * 0.6) {
      goPage(page + 1); showHint("下一頁 →");
    }
  }
  ...
  function goPage(p) {
    ...
    if (cur) {
      ...
      setCur(updated); setBooks(prev => prev.map(b => b.id === cur.id ? updated : b)); dbPut(updated);
    }
  }
    ...
    <div style={{ minHeight: "100vh", background: bg, color: tc, fontFamily: "Georgia,'Noto Serif TC',serif" }}>
          ...
          <div style={{ padding: "16px 20px", maxHeight: 360, overflowY: "auto" }}>
            {CHANGELOG.map((log, i) => (
              <div key={i} style={{ marginBottom: 20 }}>
                ...
                {log.notes.map((n, j) => (
                  <div key={j} style={{ fontSize: 13, color: tc, padding: "4px 0 4px 12px", borderLeft: `2px solid ${i === 0 ? ac : bd}`, marginBottom: 4 }}>{n}</div>
                ))}
              </div>
            ))}
          </div>
      ...
      <div style={{ padding: "20px 16px 12px", borderBottom: `1px solid ${bd}`, display: "flex", alignItems: "center", justifyContent: "space-between", background: hd }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <button style={{ ...ib, padding: "4px 6px", fontSize: 16, opacity: 0.7 }} onClick={() => setShowInfo(true)} title="版本資訊">ℹ️</button>
        </div>
  ...
  return (
    <div style={{ background: bg, color: tc, fontFamily: "Georgia,'Noto Serif TC',serif", display: "flex", flexDirection: "column", height: "100vh", width: "100vw", overflow: "hidden", filter: `brightness(${bright})` }}
      ...
      {/* Header */}
      <div style={{ padding: "10px 16px", borderBottom: `1px solid ${bd}`, display: "flex", alignItems: "center", justifyContent: "space-between", background: hd, flexShrink: 0 }}>
      ...
      <div style={{ height: 2, background: bd, flexShrink: 0 }}>
        <div style={{ height: "100%", background: ac, width: `${progressPct}%`, transition: "width 0.2s" }} />
      </div>
      ...
      {/* 底部進度資訊（取代上一頁/下一頁按鈕） */}
      <div style={{ padding: "10px 20px", borderTop: `1px solid ${bd}`, background: hd, display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        <span style={{ fontSize: 12, color: mu }}>第 {page + 1} 頁，共 {pages} 頁</span>
        ...
        <span style={{ fontSize: 12, color: ac, fontWeight: "bold" }}>{progressPct}%</span>
      </div>
